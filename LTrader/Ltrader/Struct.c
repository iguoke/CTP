...
ElemType  IOQueuePop(IOQueue *Q)
{
 ...
 (*Q).length--;
/* if (Q->length*4==Q->size &&Q->size>3*sizeof(ElemType))
 {
  ElemType *newbase;
  newbase=(ElemType *)malloc((*Q).size*sizeof(ElemType)/2);
 if(! newbase)
 {
   printf("Insufficient_memory\n");
   exit(EXIT_FAILURE);
 }
   memmove(newbase,(*Q).base+(*Q).front,sizeof(ElemType));
  size_t start=((*Q).front+1)%(*Q).size;
  if (Q->rear+1-Q->length>=0)
  {
  // printf("haha%d %d",Q->rear,Q->front);
   memmove(newbase+1,(*Q).base+start,((*Q).rear-(*Q).front)*sizeof(ElemType));
  }
  else 
  {
  // printf("hehe\n");
